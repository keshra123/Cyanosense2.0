...
path, dirs, files = next(os.walk("/Users/keshra123/Results/May 2nd Location 2 Chapman/May2ndReadings2/"))
...
for i in range(file_count):
    temp_df = pd.read_csv("/Users/keshra123/Results/May 2nd Location 2 Chapman/May2ndReadings2/"+files[i], sep='delimiter', header=None, engine='python')
    res = []
    res1 = [] 
...
for i in range(len(Epanel)):    
    ...
    Rrs5.append(value5)
    value6 = (Eup[i] * (0.02 * Edwn[i])) / (1.01 * math.pi * Epanel3[i])
    Rrs6.append(value6)
    value7 = (Eup1[i] * (0.02 * Edwn1[i])) / (1.01 * math.pi * Epanel3[i])
    Rrs7.append(value7)
